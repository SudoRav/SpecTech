...
namespace MaterialSkinExample
{
    public partial class MainForm : MaterialForm
    {
        ...
        public MainForm()
        {
			...
			seedListView();
			//materialSingleLineTextField3.Text = Properties.Settings.Default.pr1;
			if (Properties.Settings.Default.connectionString == "")
			{
				if(MessageBox.Show("Нет подключенной БД. Перейти к настройким?", "Ошибка", MessageBoxButtons.YesNo, MessageBoxIcon.Error) == DialogResult.Yes)
                {
        ...
        {
			//MessageBox.Show("1");
			//Properties.Settings.Default.pr1 = materialSingleLineTextField3.Text.ToString();

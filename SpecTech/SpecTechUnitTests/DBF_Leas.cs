    ...
    class DBF_Leas
    {
        ...
        static public string[] detail(int id)
        {
            ...
            try
            {
                ...
                cmd = new SqlCommand($"SELECT * FROM leas WHERE ID = '{id}'", con);
                adapter.SelectCommand = cmd;
                adapter.Fill(tbl);

                if (tbl.Rows.Count != 0)
                {
                    cmd = new SqlCommand($"SELECT ID FROM leas      WHERE ID = '{id}'", con);
                    StatLeas.ID = Convert.ToInt32(cmd.ExecuteScalar());

                    cmd = new SqlCommand($"SELECT address FROM leas WHERE ID = '{id}'", con);
                    StatLeas.address = cmd.ExecuteScalar().ToString();

                    cmd = new SqlCommand($"SELECT summ FROM leas    WHERE ID = '{id}'", con);
                    ...
                    cmd = new SqlCommand($"SELECT users.F + ' ' + users.I FROM leas " +
                                         ...
                                         $"WHERE leas.ID = '{id}'", con);
                    StatLeas.FI_user = cmd.ExecuteScalar().ToString();

                    //new DetailLeas().Show()
                    string[] result = new string[] { StatLeas.ID.ToString(), StatLeas.address, StatLeas.summ, StatLeas.FI_user };
                    return result;
                }
                return null;
            }
        ...
        static public bool crtLeas(int ID_user, string address, string data_start, string data_fin, string summ)
        {
            Con();
            try
            {
        ...
        static public bool delLeas(int id)
        {
            Con();
            try
            {
        ...
        static public bool delAttached(int id)
        {
            Con();
            try
            {
                ...
                cmd = new SqlCommand($"UPDATE tech SET status_leas = 'False' WHERE tech.ID = '{id}'", con);
                cmd.ExecuteNonQuery();
        ...
        static public bool crtAttached(string ID_tech)
        {
            Con();
            try
            {
